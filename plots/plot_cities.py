...
def plot_eta(df, algs, bin_size, axis = None, legend_pos = "upper left"):
    ...
    for alg in algs:
        if PARAM in df and "Robust" in alg:
            ...
            print(df[[alg,BIN,PARAM]].groupby([BIN,PARAM]).std().unstack(PARAM).min())
            print(df[[alg,BIN,PARAM]].groupby([BIN,PARAM]).std().unstack(PARAM).max())
            ...
            print(df[[alg,BIN]].groupby([BIN]).std().min())
            print(df[[alg,BIN]].groupby([BIN]).std().max())
    ...
    plt.legend(["Blocking", "hDFS", "NN", "DFS", "FP", "$\overline{R}$(FP, 0.25)", "$\overline{R}$(FP, 0.5)", "$\overline{R}$(FP, 0.75)", "$\overline{R}$(FP, 1.0)", "$\overline{R}$(FP, 1.25)"], ncol=2, loc=legend_pos)
    if axis:
        plt.axis(axis)
    plt.tight_layout()
    fig = plt.gcf()
